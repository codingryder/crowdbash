...
export function PitchWelcomeView({ roomId, roomName, sport: _sport, onComplete }: PitchWelcomeViewProps) {
  ...
  function setPower(idx: number, newVal: number) {
    ...
    if (delta > 0) {
      ...
      for (let s = 0; s < 50 && remaining > 0; s++) {
        let ri = -1, rv = MIN_B;
        for (let i = 0; i < 11; i++) {
          if (i !== idx && slots[i] && pw[i] > rv) { rv = pw[i]; ri = i; }
        }
        if (ri === -1) { pw[idx] -= remaining; break; }
        const take = Math.min(remaining, pw[ri] - MIN_B);
        pw[ri] -= take;
        remaining -= take;
      }
    } else {
      ...
      for (let s = 0; s < 50 && surplus > 0; s++) {
        let pi = -1, pv = MAX_B;
        for (let i = 0; i < 11; i++) {
          if (i !== idx && slots[i] && pw[i] < pv) { pv = pw[i]; pi = i; }
        }
        if (pi === -1) { pw[idx] += surplus; break; }
        const give = Math.min(surplus, MAX_B - pw[pi]);
        pw[pi] += give;
        surplus -= give;
      }
    }
  ...
  function applyPreset(key: string) {
    ...
    if (key === 'balanced') {
      ...
      placed.filter(x => !x.p!.player_role.toLowerCase().includes(targetRole)).forEach(({ i }) => {
        if (extra <= 0) return;
        const add = Math.min(MAX_B - pw[i], extra);
        pw[i] += add;
        extra -= add;
      });
    }
  ...
  const roleColor = (role: string) => {
    ...
    if (r.includes('bat')) return { tag: 'BAT', cls: 'rl-bat', color: 'var(--green)' };
    if (r.includes('bowl')) return { tag: 'BOWL', cls: 'rl-bowl', color: 'var(--purple)' };
    if (r.includes('all')) return { tag: 'AR', cls: 'rl-ar', color: 'var(--amber)' };
    if (r.includes('keep') || r.includes('wk')) return { tag: 'WK', cls: 'rl-wk', color: 'var(--muted)' };
    return { tag: '?', cls: '', color: 'var(--muted)' };
  };
  ...
  const hint = phase === 1 ? 'CLICK PLAYER · THEN CLICK A POSITION' : 'SQUAD LOCKED · ADJUST POWER IN RIGHT PANEL';
    ...
    <div style={{ paddingTop: 60, height: '100vh', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
      ...
      <div style={{ height: 52, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 24px', borderBottom: '1px solid var(--border)', background: 'var(--bg2)', flexShrink: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 0 }}>
          {[{ n: 1, label: 'Pick your 11' }, { n: 2, label: 'Assign power' }, { n: 3, label: 'Lock in' }].map((step, i) => (
            <div key={step.n} style={{ display: 'flex', alignItems: 'center' }}>
              {i > 0 && <span style={{ color: 'var(--faint)', fontSize: 13, padding: '0 8px' }}>›</span>}
              <div style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '0 14px', fontSize: 12, fontWeight: 600, color: phase === step.n ? 'var(--green)' : phase > step.n ? 'var(--faint)' : 'var(--muted)' }}>
                <div style={{
                  width: 20, height: 20, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center',
                  fontFamily: "'Cabinet Grotesk', sans-serif", fontSize: 9, fontWeight: 800,
                  border: phase === step.n ? 'none' : '1.5px solid currentColor',
                  background: phase === step.n ? 'var(--green)' : phase > step.n ? 'var(--faint)' : 'transparent',
                  color: phase === step.n ? '#071a0e' : 'inherit',
                }}>
                  {step.n}
                </div>
                {step.label}
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <div style={{ fontSize: 12, color: 'var(--muted)' }}>
            {phase === 1 && <><b style={{ fontFamily: "'Cabinet Grotesk', sans-serif", color: 'var(--green)' }}>{pickCount}</b>/11 picked</>}
            {phase === 2 && <><b style={{ fontFamily: "'Cabinet Grotesk', sans-serif", color: isBalanced ? 'var(--green)' : 'var(--amber)' }}>{totalUsed}</b>/33 power</>}
          </div>
          <button
            onClick={handleNext}
            disabled={(phase === 1 && pickCount < 11) || saving || phase === 3}
            style={{
              ...
              opacity: ((phase === 1 && pickCount < 11) || saving || phase === 3) ? 0.3 : 1,
            }}
          >
            {saving ? '...' : phase === 1 ? 'Assign Power →' : phase === 2 ? 'Lock in & go live →' : 'Squad live ✓'}
          </button>
        </div>
      </div>
        ...
        <div style={{ borderRight: '1px solid var(--border)', display: 'flex', flexDirection: 'column', overflow: 'hidden', background: 'var(--bg2)' }}>
          ...
          <div style={{ padding: '8px 15px', flexShrink: 0 }}>
            <input
              ...
              style={{ width: '100%', background: 'var(--surface)', border: '1px solid var(--border)', borderRadius: 8, padding: '7px 10px', fontSize: 12, color: 'var(--text)', outline: 'none', boxSizing: 'border-box' }}
            />
          </div>
          <div style={{ display: 'flex', gap: 4, padding: '0 15px 8px', flexShrink: 0, flexWrap: 'wrap' }}>
            {['all', 'bat', 'bowl', 'ar', 'wk'].map(r => (
              <button key={r} onClick={() => setRoleFilter(r)} style={{
                fontSize: 10, padding: '3px 9px', borderRadius: 100, cursor: 'pointer',
                fontFamily: "'Cabinet Grotesk', sans-serif", fontWeight: 700, transition: 'all 0.15s',
                border: roleFilter === r ? '1px solid rgba(45,214,122,0.25)' : '1px solid var(--border)',
                background: roleFilter === r ? 'rgba(45,214,122,0.08)' : 'transparent',
                color: roleFilter === r ? 'var(--green)' : 'var(--muted)',
              }}>
            ...
            {teams.map(team => (
              <div key={team}>
                ...
                {benchPlayers.filter(p => p.team === team).map(player => {
                  ...
                  const rc = roleColor(player.player_role);
                  const rcBg = getRoleColorBg(player.player_role);
                  const initials = player.player_name.split(' ').map(w => w[0]).join('').slice(0, 2).toUpperCase();

                  return (
                    <div
                      key={player.player_id}
                      onClick={() => !isPlaced && handleBenchClick(player)}
                      style={{
                        display: 'flex', alignItems: 'center', gap: 8,
                        padding: '8px 10px', borderRadius: 9,
                        border: isSel ? '1px solid var(--green)' : '1px solid var(--border)',
                        background: isSel ? 'rgba(45,214,122,0.05)' : 'var(--surface)',
                        marginBottom: 5, cursor: isPlaced ? 'default' : 'pointer',
                        transition: 'all 0.18s',
                        opacity: isPlaced ? 0.28 : 1,
                        pointerEvents: isPlaced ? 'none' : 'auto',
                      }}
                    >
                      <div style={{ width: 30, height: 30, borderRadius: 7, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: "'Cabinet Grotesk', sans-serif", fontSize: 9, fontWeight: 700, flexShrink: 0, background: rcBg.bg, color: rcBg.color }}>
                        {initials}
                      </div>
                      ...
                      <span style={{ fontSize: 9, fontFamily: "'Cabinet Grotesk', sans-serif", fontWeight: 700, borderRadius: 3, padding: '1px 5px', flexShrink: 0, ...getRoleColorBg(player.player_role) }}>
                        {rc.tag}
                      </span>
                    </div>
                  );
        ...
        <div style={{ borderLeft: '1px solid var(--border)', display: 'flex', flexDirection: 'column', overflow: 'hidden', background: 'var(--bg2)' }}>
          ...
          <div style={{ display: 'flex', gap: 5, padding: '8px 15px', borderBottom: '1px solid var(--border)', flexShrink: 0, flexWrap: 'wrap' }}>
            {[{ key: 'balanced', icon: '⚖️', label: 'Reset' }, { key: 'top3bat', icon: '🏏', label: 'Batters' }, { key: 'bowlers', icon: '🎯', label: 'Bowlers' }, { key: 'allrounder', icon: '⚡', label: 'All-rds' }].map(p => (
              <button key={p.key} onClick={() => applyPreset(p.key)} style={{
                padding: '4px 9px', borderRadius: 100, border: '1px solid var(--border)', background: 'transparent',
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '10px 15px' }}>
            {slots.map((player, i) => {
              ...
              return (
                <div key={player.player_id} style={{ background: 'var(--surface)', border: `1px solid ${maxed ? 'rgba(240,82,82,0.22)' : 'var(--border)'}`, borderRadius: 10, padding: '9px 12px', marginBottom: 7, transition: 'border-color 0.2s' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 7, marginBottom: 7 }}>
                    <div style={{ width: 26, height: 26, borderRadius: 6, display: 'flex', alignItems: 'center', justifyContent: 'center', fontFamily: "'Cabinet Grotesk', sans-serif", fontSize: 8, fontWeight: 700, flexShrink: 0, ...rcBg }}>
                      {initials}
                    </div>
                    <div style={{ flex: 1, fontSize: 11, fontWeight: 500, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>
                      {player.player_name.split(' ').pop()}
                    </div>
                    <div style={{ fontFamily: "'Cabinet Grotesk', sans-serif", fontSize: 15, fontWeight: 900, minWidth: 24, textAlign: 'right', color: maxed ? 'var(--red)' : minned ? 'var(--blue)' : 'var(--amber)' }}>
                      {pw}x
                    </div>
                  </div>
                  {/* Slider */}
                  <div
                    ...
                    onMouseDown={(e) => {
                      ...
                      const handle = (clientX: number) => {
                        ...
                        const val = Math.round(MIN_B + pctX * (MAX_B - MIN_B));
                        setPower(i, val);
                      };
                      handle(e.clientX);
                  ...
                  >
                    ...
                    <div style={{
                      position: 'absolute', left: `${pct}%`, width: 18, height: 18, borderRadius: '50%',
                      border: '2.5px solid rgba(0,0,0,0.35)', transform: 'translateX(-50%)', pointerEvents: 'none',
                      background: maxed ? 'var(--red)' : minned ? 'var(--blue)' : 'var(--amber)', transition: 'background 0.15s',
                    }} />
                  </div>
                </div>
              );
